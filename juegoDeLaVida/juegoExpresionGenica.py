...
def main():
    print(
        ...
        "Buena suerte!!\n")
    respuesta = input("Ingresa cualquier tecla para iniciar el juego o S para salir: \n")
...
def acoplarseAlADN():
    respuesta = input(
        "\nIngresá A para ir directo a copiar una hebra de ADN, o ingresá B para deshacer la hélice de ADN:\n")
    while respuesta != "B":
        chequearSalida(respuesta)
        if respuesta == "A":
            respuesta = input(
...
def encontrarGen():
    respuesta = input(
        ...
        "Ingresá A para buscar la región promotora que da inicio al gen que estás buscando, o ingresa B para "
        "intentar plegarte en cualquier lugar de la cadena de ADN (hoy te sentís con suerte!) \n")
    while respuesta != "A":
        chequearSalida(respuesta)
        if respuesta == "B":
            respuesta = input(
    ...
    corroborarCorrespondenciaNucleotidos()


def corroborarCorrespondenciaNucleotidos():
    respuesta = input(
        "\nGenial, encontraste la región promotora y por lo tanto el inicio del gen cuyos nucleótidos debes "
        "transcribir. Antes de seguir, corroboramos que sepas cómo vas a hacer la transcripción! Por ejemplo, si una "
        "secuencia de la hebra molde del ADN a transcribir es 'ACCATCAG', ¿cuál de las siguientes secuencias de ARN "
        "es correcta? A: 'TGGTAGTC' o B: 'UGGUAGUC'\n")
    while respuesta != "B":
        chequearSalida(respuesta)
        if respuesta == "A":
            respuesta = input(
...
def iniciarTranscripcion():
    respuesta = input(
        ...
        "Una vez allí: A, decidis llevar vos misma la información del gen hasta la próxima etapa (no confiás en nadie "
        "más para este trabajo!); o B, sintentizás una cadena de ARN mensajero y transcribís allí la información "
        "(que viva el trabajo en equipo!) \n")
    while respuesta != "B":
        chequearSalida(respuesta)
        if respuesta == "A":
            respuesta = input(
...
def encontrarRibosoma():
    respuesta = input(
        "\nComo ARN mensajero, debes encontrar un ribosoma, que construirá la proteína en base a la información "
        "que llevás. Escribí A para salir del núcleo celular, o B para buscar un ribosoma dentro del núcleo. \n")
    while respuesta != "A":
        chequearSalida(respuesta)
        if respuesta == "B":
            respuesta = input(
...
def ingresarAlRibosoma():
    respuesta = input(
        ...
        "pueda leer toda tu secuencia de a tres nucleótidos a la vez (lo que se denomina codón). Pero momento, ¿cómo "
        "sabrá en qué codón de tu secuencia debe empezar a traducir? Escribí A para que el ribosoma arranque al "
        "principio de tu cadena, o B para que busque un codón de inicio dentro de tu cadena: \n")
    while respuesta != "B":
        chequearSalida(respuesta)
        if respuesta == "A":
            respuesta = input(
    ...
    buscarARNDeTraduccion()


def buscarARNDeTraduccion():
    respuesta = input(
        ...
        "(lo que denominamos traducción). ¿Quién hará esta tarea? Indica A si crees que será otra organela, o B "
        "si crees que será el ARN de transferencia: \n")
    while respuesta != "B":
        chequearSalida(respuesta)
        if respuesta == "A":
            respuesta = input(
...
def finalizarTraduccion():
    respuesta = input(
        ...
        "al siguiente codón. ¿Cuándo termina este proceso? Ingresá A si crees que el ribosoma da la señal de fin, "
...
def chequearSalida(respuesta: str):
    if respuesta == "S":
